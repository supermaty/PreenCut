...
class LLMProcessor:
    ...
    def segment_video(self, subtitles: Any,
        ...
        system_prompt = (
            ...
            品牌与产品：
            派星 -> 派心、派新、派芯、派薪、泰星、泰心、泰芯、泰薪、泰欣、泰鑫；
            ...
            - 何时应另起新段：话题明显切换、换产品/换观众、长时间停顿或冷场时，应开始新片段；单段不宜过长，尽量不超过本片字幕总时长的 30%，以利于后续剪辑与检索。
            ...
            - 若内容中提及派星/合生元派星仅作为与其它产品的对比，且最终并未推荐派星（例如推荐了别的奶粉或让观众选别的），则该段不要保留；只保留真正推荐或正面介绍派星/合生元派星的片段。
            - 排除纯口播废话、引导语、无实质内容的短句，例如：「关注榜一小助理」「点关注不迷路」「给主播点点赞」「大家帮忙扣个1」「感谢某某送礼物」等仅用于互动引流、与品牌/产品无关的句子或段落，不要出现在结果中。
        ...
        # 用户提示（如果有自定义提示则使用；默认与 system 的「宁多勿少」一致，不限制片段数量）
        user_prompt = prompt or (
            "请根据以下字幕内容，按主题与语境切分成若干有意义片段。"
            "每个片段应包含连贯的主题内容，给出一句话摘要和1-3个主题标签，时间精确到秒。"
            "片段数量按实际内容决定，可多于 10 段，执行「宁多勿少」、保证内容完整。"
        )
